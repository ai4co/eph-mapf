...
class GlobalBuffer:
    def __init__(self, buffer_capacity=config.buffer_capacity, init_env_settings=tuple(config.init_env_settings),
        ...
        if config.use_wandb:     
            wandb.init(project=config.project, name=config.name, id=config.run_id, config=dict(config))
            self.wandb = wandb
        ...
        if config.use_wandb:
            ...
            # for num_agents in range(config.init_env_settings[0], config.max_num_agents+1, 4):
                for map_len in range(config.init_env_settings[1], config.max_map_lenght+1, 5):
                    ...
                    if (num_agents, map_len) in self.stat_dict:
                        self.wandb.log({"train/"+fmt.format(num_agents, map_len): sum(self.stat_dict[(num_agents, map_len)])/(len(self.stat_dict[(num_agents, map_len)]) + 1e-10) })
                    else:
...
class Learner:
    def __init__(self, buffer: GlobalBuffer):
        ...
        self.scheduler = MultiStepLR(self.optimizer, milestones=config.lr_scheduler_milestones, gamma=config.lr_scheduler_gamma)
        self.buffer = buffer
        self.counter = 0
        self.last_counter = 0
        self.done = False
    ...
    def _train(self):
        ...
        for i in range(1, config.training_steps+1):
            ...
            if i % config.val_interval == 0:
                assert checkpoint_path is not None, "A checkpoint path must be saved before validation"
    ...
    def validate(self, checkpoint_path, step, run_id, name):
        ...
        command = ["python", "validate.py", "--checkpoint_path", checkpoint_path, "--step", str(step), "--run_id", run_id, "--name", name]
        print(" ".join(command)) 
