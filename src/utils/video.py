...
def save_frame(
    frame_index, map_, init_pos, goal_pos, positions, frames_dir="/tmp/agent_frames/"
):
    ...
    ax.hlines(
        y=np.arange(0.5, map_.shape[0] - 1.5),
        xmin=0,
        xmax=map_.shape[1] - 0.5,
        color="k",
        linewidth=0.5,
        alpha=0.5,
    )
    ax.vlines(
        x=np.arange(0.5, map_.shape[1] - 1.5),
        ymin=0,
        ymax=map_.shape[0] - 0.5,
        color="k",
        linewidth=0.5,
        alpha=0.5,
    )
    ...
    for i in range(num_agents):
        ...
        ax.plot(
            positions[frame_index, i, 1],
            positions[frame_index, i, 0],
            "o",
            color=colors[i],
            markersize=5,
            label=f"Agent {i+1} Position" if frame_index == 0 else "",
        )
...
def create_video(
    ...
    args_list = [
        (i, map_, init_pos, goal_pos, positions, frames_dir)
    ...
    ffmpeg_cmd = [
        ...
        "10",  # Set frame rate
        "-i",
        os.path.join(frames_dir, "frame_%04d.png"),  # Input frames format
        "-c:v",
        "libx264",  # Codec to use
        "-pix_fmt",
