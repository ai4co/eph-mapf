...
class Environment:
                ...
                if len(collide_agent_id) > 1:
                    ...
                    if all_in_checking:
                        ...
                        for pos, id in zip(collide_agent_pos, collide_agent_id):
                            pos.append(id)
                        collide_agent_pos.sort(key=lambda x: x[0]*self.map_size[0]+x[1]) # TODO (or actually fine)
    ...
    def conflict_checker(self, actions, q_val):
        ...
        for agent_id in range(self.num_agents):
            ...
            while self.is_unavailable_action(self.agents_pos[agent_id][0], self.agents_pos[agent_id][1], actions[agent_id], self.map):
                count+=1
                if count>4:
                    actions[agent_id] = initial_action
                    #generate a warning
                    print("Warning: agent {} is stuck in an islolated cell".format(agent_id))
                    break
            ...
            for agent_id in checking_list:
                ...
                if target_agent_id.size > 0:
                    ...
                    if np.array_equal(next_pos[target_agent_id], self.agents_pos[agent_id]):
                        ...
                        if q_val[agent_id][actions[agent_id]] < q_val[target_agent_id][actions[target_agent_id]]:
                            q_val[agent_id][actions[agent_id]] = -np.inf
                            actions[agent_id] = torch.argmax(q_val[agent_id]).item()
                            #move agent_id
                            next_pos[agent_id] = self.agents_pos[agent_id] + ACTION_LIST[actions[agent_id]]

                            # checking_list.remove(target_agent_id)
                        else:
                            q_val[target_agent_id][actions[target_agent_id]] = -np.inf
                            actions[target_agent_id] = torch.argmax(q_val[target_agent_id]).item()
                            #move target_agent_id
                            next_pos[target_agent_id] = self.agents_pos[target_agent_id] + ACTION_LIST[actions[target_agent_id]]
        ...
        while not no_conflict:
            ...
            for agent_id in checking_list:
                ...
                if target_agent_id.size > 0:
                    ...
                    if np.array_equal(next_pos[target_agent_id], self.agents_pos[agent_id]): #and target_agent_id != agent_id:
                        assert target_agent_id in checking_list, 'target_agent_id should be in checking list'
    ...
    def advanced_escape_policy(self, next_pos, q_val, loc_history, actions):
        ...
        for agent_id, iter_ in enumerate(agent_idx_priority_escape):
            if mask_final[agent_id]:
                ...
                if config.aep_distance_threshold is not None:
                    if np.linalg.norm(self.agents_pos[agent_id] - self.goals_pos[agent_id]) < config.aep_distance_threshold:
                        continue
                ...
                if config.aep_use_q_value_only:
                    ...
                    path=self.astar(upd_map, self.agents_pos[agent_id], self.goals_pos[agent_id], next_pos, self.aep_astar_type)
                    try:
                        _ = path[1][0]
                        use_q_value = False
                    except:
                ...
                if not use_q_value:
                    ...
                    for action in range(5):
                        if not self.is_unavailable_action(self.agents_pos[agent_id][0], self.agents_pos[agent_id][1], action, upd_map):
        ...
        actions = [DIRECTION_TO_ACTION[(next_pos[i][0] - self.agents_pos[i][0], next_pos[i][1] - self.agents_pos[i][1])] for i in range(self.num_agents)]
   ...
   #########################################
    ...
    def step_infer(self, q_val, no_active_agent_nearby, loc_history):
        ...
        if self.astar_type is not None:
            ...
            for agent_id in range(self.num_agents):
                if no_active_agent_nearby[agent_id][0] and not np.array_equal(self.agents_pos[agent_id], self.goals_pos[agent_id]):
                    path=self.astar(self.map, self.agents_pos[agent_id], self.goals_pos[agent_id], self.agents_pos)
                    if path is not None:
                        pos=path[0][0]
                        new_pos=path[1][0]
                        direction=(new_pos[0] - pos[0], new_pos[1] - pos[1])
        ...
        if self.use_aep:
            next_pos, loc_history = self.advanced_escape_policy(next_pos, q_val_init, loc_history, actions)
        ...
        assert np.unique(self.agents_pos, axis=0).shape[0] == self.num_agents, 'agent overlapping'
    ...
    def is_unavailable_action(self, curr_x, curr_y, action, map_=None):
        ...
        if next_x < 0 or next_x >= self.map_size[0] or next_y < 0 or next_y >= self.map_size[1]:
            return True
    ...
    def observe_infer(self):
        ...
        obs_finished_or_no = np.zeros((self.num_agents, 2*self.active_agent_radius+1, 2*self.active_agent_radius+1), dtype=bool)
        ...
        agent_map = np.zeros((self.map_size), dtype=bool)
        agent_map[self.agents_pos[:,0], self.agents_pos[:,1]] = 1
        agent_map = np.pad(agent_map, self.obs_radius, 'constant', constant_values=0)  # pad with 0, so later...
        ...
        unfinished_agents_map = np.pad(unfinished_agents_map, self.active_agent_radius, 'constant', constant_values=0)
        ...
        for i, agent_pos in enumerate(self.agents_pos):
            ...
            obs[i, 1] = obstacle_map[x:x+2*self.obs_radius+1, y:y+2*self.obs_radius+1]
            obs[i, 2:] = self.heuri_map[i, :, x:x+2*self.obs_radius+1, y:y+2*self.obs_radius+1]
            obs_finished_or_no[i] = unfinished_agents_map[x:x+2*self.active_agent_radius+1, y:y+2*self.active_agent_radius+1]
        ...
        return obs, np.copy(self.last_actions), np.copy(self.agents_pos), no_active_agent_nearby
    ...
    def astar(self, map, start, goal, agent_pos, astar_type=None):
        ...
        if astar_type is None:
            astar_type = self.astar_type if self.astar_type is not None else 1 # default to 1
        if astar_type == 1:
            ...
            raise ValueError('Invalid astar_type: {}'.format(self.astar_type) ,
    ...
    def _astar2(self, map, start, goal, agent_pos):
        ...
        agents_to_remove = []
        for i in range(self.num_agents):
            if np.array_equal(start, robots[i]) or not np.array_equal(self.goals_pos[i], robots[i]):
                agents_to_remove.append(i)

        # Create a new list that contains only the agents you want to keep
        filtered_robots = [robots[i] for i in range(len(robots)) if i not in agents_to_remove]
    ...
    def _astar_main(self, world, start, goal):
        try:
            ...
            if isinstance(exception, NoSolutionError) or isinstance(exception, OutOfTimeError):
                path = None
            else:
                path = None
