...
class CommLayer(nn.Module):
    ...
    def position_embed(self, relative_pos, dtype, device):
        ...
        return position_embedding  # size: batch_size*num_agents*num_agents x pos_embed_dim
    ...
    def step(self, obs, last_act, pos):
        ...
        if self.selective_comm:
            ...
            if self.hidden is None:
                test_hidden = torch.zeros((num_in_obs_agents.sum(), self.hidden_dim)).to(obs.device)
            else:
                test_hidden = torch.repeat_interleave(self.hidden, num_in_obs_agents, dim=0).to(obs.device)
            ...
            actions_mat = torch.ones((num_agents, num_agents), dtype=test_actions.dtype, device = obs.device) * -1
        ...
        return actions, q_val.cpu().numpy(), self.hidden.squeeze(0).cpu().numpy(), relative_pos.cpu().numpy(), comm_mask.cpu().numpy() 
